...
The authentication code calculation is done based on the portion of the packet's buffer showed above (from the first byte after the packet size until the last byte of the encrypted payload). The calculation also takes a auth. key which is defined at the beginning of the networking session. It will be better detailed in the [last section]().
